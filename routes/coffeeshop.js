var add = function(req, res){
    ...
    console.log('파라미터 : ' + paramName + ', ' + paramAddress + ', ' 
...
var list = function(req,res){
    ...
    //static으로 등록한 함수 findAll이 실행된다.
    database.CoffeeshopModel.findAll(function(err, results){
        if(err){
            res.writeHead(200, {'Content-Type':'text/html;charset=utf8'});
            res.write('<h2>커피숍 조회중 에러 발생</h2>');
            res.end();
        ...
        if(results){
            ...
            for(var i = 0; i<results.length; i++){
                ...
                + ', ' + curTel + ', ' + curLongitude + ', ' + curLatitude);
            }
            res.write('</ul></div>');
...
var findNear = function(req,res){
    console.log('findNear 라우팅함수 호출됨.');
    ...
    database.CoffeeshopModel.findNear(paramLongitude, paramLatitude, maxDistance, function(err,results){
        if(err){
            res.writeHead(200, {'Content-Type':'text/html;charset=utf8'});
            res.write('<h2>가까운 커피숍 조회중 에러 발생</h2>');
            res.end();
